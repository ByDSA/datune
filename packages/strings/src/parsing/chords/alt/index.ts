...
import { fromRootIntervalSet } from "@datune/core/chords/octave/alt/building/root-intervalSet";
...
import { parseIntervalSet } from "parsing/intervalSets/alt";
...
class ChordAltString extends ChordStringAbstract<Chord> {
  ...
  protected parsingNormal(): Chord | null {
    const parser = new ParserBottomUp()
      ...
      .add(
        IntervalSet.name,
        (str: string): IntervalSet | null => parseIntervalSet(str, this.options),
      );
